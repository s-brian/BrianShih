...
private val IMAGE_SIZE_DP = 100.dp
...
fun ExamScreen(examViewModel: ExamViewModel = viewModel()){
    ...
    Box(
        ...
        Box(
            modifier = Modifier
                .size(IMAGE_SIZE_DP)
                .offset(x = examViewModel.circleX.dp, y = examViewModel.circleY.dp)
                .pointerInput(Unit) {
            ...
            Image(
                painter = painterResource(id = examViewModel.selectedImageId),
                contentDescription = "Randomly selected item",
                modifier = Modifier.fillMaxSize()
            )
        }
    }
